...
INPUT_FILE = r"bhumisetu\output\output_of_classifier\bhumi_rasid_extraction.json"
OUTPUT_FILE = BASE_DIR / "label_extraction_with_confidence.json"
...
def main():
    ...
    for field in FIELDS:
        ...
        value = final_fields.get(
            field
        )

        confidence = final_confidence.get(
            field,
            0.0
        )
        ...
        print(
            ...
            f"{value} "
            f"[{confidence:.2f}% "
            f"{confidence_status(confidence)}]"
        )

    print(
        "\nStage confidence:",
        output["stage_confidence"]
    )
