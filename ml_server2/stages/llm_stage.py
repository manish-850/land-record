...
# ==========================================
# Gemini setup (lazy singleton client)
#
# The client is only created the first time extract_paragraph() is
# actually called, so importing this module does not require
# GEMINI_API_KEY to already be set (useful if the LLM stage is
# disabled for a given pipeline run).
# ==========================================
    ...
    if _client_state["client"] is None:
        with _client_lock:
            if _client_state["client"] is None:
                ...
                _client_state["client"] = genai.Client(api_key=api_key)
...
def extract_llm(classified_data, verbose=False):
    """
    Runs Gemini-based extraction over every region routed as
    "paragraph" by the classifier stage.
    ...
    Returns:
        A list of per-region result dicts:
            {
                "region_id": ...,
                "ocr_text": ...,
                "ocr_confidence": ...,
                ...
                "confidence_status": {field: status, ...},
                "stage_confidence": {"score": ..., "status": ...}
            }
    """

    results = []

    for region in classified_data.get("regions", []):
        ...
        stage_confidence = (
            round(sum(extracted_scores) / len(extracted_scores), 2)
            if extracted_scores
            else 0.0
        )

        stage_status = confidence_status(stage_confidence)
        ...
        if verbose:
            ...
            for field, score in field_confidence.items():
                print(f"{field:22}: {score:6.2f} ({confidence_statuses[field]})")

            print("\nOCR confidence:", ocr_confidence)
            print("LLM stage confidence:", stage_confidence, f"({stage_status})")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run LLM extraction on classifier output")
    parser.add_argument("classifier_json_path")
    parser.add_argument("--output", default="paragraph_extraction.json")
    args = parser.parse_args()

    with open(args.classifier_json_path, "r", encoding="utf-8") as f:
        classified_data = json.load(f)

    results = extract_llm(classified_data, verbose=True)
