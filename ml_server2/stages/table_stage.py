...
def extract_tables(classified_data, verbose=False):
    """
    Runs table extraction over every region routed as "table" by the
    classifier stage.
    ...
    Returns:
        A list of per-table-region result dicts (see extract_table()),
        one per region that yielded at least one non-null field.
    """
    ...
    if verbose:
        ...
        for result in table_results:
            print("\n" + "-" * 60)
            print("Region:", result.get("region_id"))
            print("Khata :", result.get("khata_no"))
            print("Khesra:", result.get("khesra_no"))
            print("Area  :", result.get("area"))
            print("Unit  :", result.get("area_unit"))
            print("Lagaan:", result.get("lagaan"))

            for field in [
                "district", "circle", "halka", "village",
                "raiyat_name", "father_husband_name", "address",
            ]:
                if result.get(field) is not None:
                    print(f"{field:22}:", result[field])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run table extraction on classifier output")
    parser.add_argument("classifier_json_path")
    parser.add_argument("--output", default="table_extraction.json")
    args = parser.parse_args()

    if not os.path.exists(args.classifier_json_path):
        raise FileNotFoundError(f"Classifier output not found:\n{args.classifier_json_path}")

    with open(args.classifier_json_path, "r", encoding="utf-8") as f:
        classified_data = json.load(f)

    table_results = extract_tables(classified_data, verbose=True)
